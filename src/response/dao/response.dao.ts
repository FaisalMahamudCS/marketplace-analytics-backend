...
export class MarketplaceResponseDAO implements IMarketplaceResponseDAO {
    ...
    async findLatest(): Promise<MarketplaceResponseDocument | null> {
        return await this.marketplaceResponseModel
            .findOne()
            .sort({ timestamp: -1 })
            .exec();
    ...
    async getAverageResponseTime(): Promise<number> {
        ...
        return (result[0] as any)?.avgTime || 0;
    }
}
...
export class GenericResponseDAO implements IGenericResponseDAO {
    ...
    async create(responseData: Record<string, unknown>): Promise<ResponseDocument> {
        const response = new this.responseModel(responseData);
        return await response.save();
    }

    async findAll(limit: number = 100, offset: number = 0): Promise<ResponseDocument[]> {
